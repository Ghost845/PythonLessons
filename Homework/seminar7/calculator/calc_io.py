...
def get_real_value(num):
    while True:
        try:
            ...
            write_log(cctrl.t(), 'Ошибка ввода вещественного числа')
            write_console('Нужно ввести число. Попробуйте ещё раз')
    return a
...
def get_complex_value(num):
    while True:
        try:
            a = int(input(f'Введите дествительную часть {num}-го числа: '))
            b = int(input(f'Введите мнимую часть {num}-го числа: '))
            break
        except ValueError:
            write_log(cctrl.t(), 'Ошибка ввода целого числа')
            write_console('Нужно ввести целое число. Попробуйте ещё раз')
    return (complex(a, b))


def input_number(num: int):
    write_console(f'Ввод {num}-го числа')
    complex_num = True if input('Число комплексное? (y/n): ') in ('y', 'Y') else False
...
def menu(complex=True):
    ...
    real_menu_keys = sorted(complex_menu | additional)[1:]+[sorted(complex_menu | additional)[0]]
    if complex:
        dic = complex_menu
        for key in dic:
